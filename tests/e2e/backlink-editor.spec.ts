...
test.describe('Backlink Editor', () => {
  test('capture console logs when using backlink button', async ({ page }) => {
    ...
    page.on('console', (msg) => {
      ...
      if (text.includes('🔧') || text.includes('🎯') || text.includes('🔥') || text.includes('Mode bouton') || text.includes('BacklinkPicker')) {
    ...
    const firstBillet = page.locator('button').filter({ hasText: 'bibliographie-collective' }).first()
