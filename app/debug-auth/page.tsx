        ...
        <div className="p-6 border rounded-lg bg-gray-50">
          ...
          {session ? (
            <div className="space-y-2 font-mono text-sm">
              ...
              <p>
                <strong>Nom:</strong> {session.user?.name || 'N/A'}
              </p>
              <p>
                <strong>ID:</strong> {(session.user as any)?.id || 'N/A'}
              </p>
              <p>
                <strong>Rôle:</strong> {(session.user as any)?.role || 'N/A'}
              </p>
            </div>
          ) : (
        ...
        <div className="p-6 border rounded-lg bg-gray-50">
          ...
          {dbUser ? (
            <div className="space-y-2 font-mono text-sm">
              ...
              <p>
                <strong>Créé:</strong> {dbUser.createdAt.toLocaleString('fr-FR')}
              </p>
              <p>
                <strong>Comptes liés:</strong>{' '}
                {accounts.map(a => `${a.provider}:${a.providerAccountId}`).join(', ') || 'Aucun'}
              </p>
            </div>
          ) : (
        ...
        <div className="p-6 border rounded-lg bg-gray-50">
          ...
          <div className="space-y-2">
            ...
            {session && !session.user?.email && (
              <p className="text-red-600">❌ Session sans email</p>
            )}
            {session && !(session.user as any)?.role && (
              <p className="text-red-600">❌ Session sans rôle</p>
            )}
            {session && (session.user as any)?.role !== 'ADMIN' && (
              <p className="text-red-600">❌ Rôle incorrect: {(session.user as any)?.role}</p>
            )}
            {dbUser && dbUser.role === 'ADMIN' && (
              <p className="text-green-600">✅ Rôle ADMIN en base</p>
            )}
            {accounts.length > 0 && <p className="text-green-600">✅ Comptes OAuth liés</p>}
            {session && (session.user as any)?.role === 'ADMIN' && (
              <p className="text-green-600">✅ Session admin correcte</p>
            )}
          </div>
        </div>
